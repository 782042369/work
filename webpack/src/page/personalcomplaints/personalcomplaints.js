import './personalcomplaints.scss'
...
$(function(){
	...
	function getList(value,page){
        ...
        http(api.getComplaintListByCusToken,data).then((res) => {
            if(res.status >= 0){
				let dat = res.data;
				getListStr(dat);
				let params = {
	...
	function getListStr(dat){
		...
		$.each(dat, function(i, comment) {
			let n = i+1;
			html += `<tr><td>${n}</td><td>${dat[i].title}</td><td>${dat[i].time}</td><td class="td_two"><a href="../page/personalinteractdetail.html?complaints&${dat[i].interactionId}" class="look">查看</a><a href="" class="remove" data-target="#myDel"  data-id="${dat[i].interactionId}">删除</a></td>`;
		});
	...
	function modal(con,value,interactionIdNum){
		layer.open({
			...
			yes: function(index, layero) { //这里写最后的逻辑和传参
				layer.close(index);
				http(api.delComplaint,{interactionId:interactionIdNum}).then((res) => {
					if(res.status >= 0) {
						getList(value,1);
					} else {
						layer.msg('删除失败')
