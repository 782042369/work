import './personalsuggestion.scss'
...
$(function(){
	...
	function getList(value,page){
        ...
        http(api.getProposalListByCusToken,data).then((res) => {
            if(res.status >= 0){
                console.log(res);
                let dat = res.data;
                getListStr(dat);
                ...
                $(document).off().on('click','.remove',function(e){
					...
					let con = `<div class="tan_qt">您确定要删除该记录么？<br />删除后，该建议在我的建议中不会出现,<br />但若该建议已在建议公开中出现，<br />该建议信息并不会删除。 </div>`;
    ...
    function getListStr(dat){
        ...
        $.each(dat, function(i, comment) {
            let n = i+1;
            html += `<tr><td>${n}</td><td>${dat[i].title}</td><td>${dat[i].time}</td><td class="td_two"><a href="../page/personalinteractdetail.html?proposal&${dat[i].interactionId}" class="look">查看</a><a href="" class="remove" data-target="myDel"  data-id="${dat[i].interactionId}">删除</a></td>`;
        });
	...
	function modal(con,value,interactionIdNum){
		layer.open({
			...
			yes: function(index, layero) { //这里写最后的逻辑和传参
				layer.close(index);
				http(api.delProposal,{interactionId:interactionIdNum}).then((res) => {
                    if(res.status >= 0){
                        let value = "";
                        $(".active").html() == "未回复" ? value = false : value = true;
