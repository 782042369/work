...
export default class GoodsCateService extends Service {
	async find() {
		try {
			let result: any = []
	...
	async editgoodscate() {
		try {
			...
			const result: any = await this.ctx.model.GoodsCate.updateOne(
				{
					_id: id
				},
	...
	async deletegoodscate() {
		...
		const result: any = await this.ctx.model.GoodsCate.deleteOne({
			_id
		})
		return result
	}
