...
export default class ArticleService extends Service {
	...
	async find() {
		...
		const result: any = await this.ctx.model.Article
	...
	async edit() {
		...
		const result: any = await this.ctx.model.Article.updateOne(
			{
				_id,
			},
	...
	async delete() {
		const _id = this.ctx.request.body.id;
		const result: any = await this.ctx.model.Article.deleteOne({
			_id,
		});
		return result;
	}
