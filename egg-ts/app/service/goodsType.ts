...
export default class GoodsService extends Service {
	async find() {
		try {
			...
			const result: any = await this.ctx.model.GoodsType
	...
	async editgoods() {
		...
		const result: any = await this.ctx.model.GoodsType.updateOne(
			{
				_id,
			},
	...
	async deletegoods() {
		const _id = this.ctx.request.body.id;
		const result: any = await this.ctx.model.GoodsType.deleteOne({
			_id,
		});
		return result;
	}
