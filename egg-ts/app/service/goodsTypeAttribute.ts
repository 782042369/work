...
export default class GoodsTypeAttributeService extends Service {
	async find() {
		try {
			let result: any = [];
	...
	async editgoodsattribute() {
		...
		const result: any = await this.ctx.model.GoodsTypeAttribute.updateOne(
			{
				_id,
			},
	...
	async deletegoodsattribute() {
		const _id = this.ctx.request.body.id;
		const result: any = await this.ctx.model.GoodsTypeAttribute.deleteOne({
			_id,
		});
		return result;
	}
