 ...
 * @LastEditors: 杨宏旋
 * @LastEditTime: 2019-02-18 11:46:00
 * @Description: 播放音乐
-->
<template>
  <ul class="music-box">
    <template v-for="(item,index) in videoslist">
      <li :key="index" @click="handleOpenMusic(index)" class="active">
        ...
        <audio
          ...
          class="player"
        ></audio>
        <div :class="!play[index]?'no':'yes'" @click="handleMusicFlag(index)" class="music-mask">
          <svg>
            <defs>
          ...
          <p v-show="activemusic === index">{{nowaudiotxt}}/{{audiotxt[index]}}</p>
          <span></span>
          <em></em>
        </div>
        <p>{{item.desc}}</p>
  ...
  methods: {
    ...
    visibilityChangeHandler(hidden) {
      ...
      if (hidden && this.activemusic !== null) {
        ...
        this.$set(this.play, this.activemusic, false);
      }
    },
    handleMusicFlag(num) {
      this.$set(this.play, num, !this.play[num]);
      const music = document.querySelectorAll('.player'); // 音频所在对象
      if (this.play[num]) {
        music[num].play(); // 播放音乐
      } else {
